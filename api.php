...
else if ($action === "getSongsStats") {
    getSongsStats($conn);
}
else if ($action === "getPurchasesStats") {
    getPurchasesStats($conn);
}
...
else if ($action === "getPurchaseDetails") {
    getPurchaseDetails($conn);
}
else if ($action === "updatePurchaseStatus") {
    updatePurchaseStatus($conn);
}
else if ($action === "generateInvoice") {
    generateInvoice($conn);
}
else if ($action === "getPurchaseStats") {
    getPurchaseStats($conn);
}
...
function login($conn) {
    ...
    $updateSql = "UPDATE usuarios SET last_login = NOW() WHERE id = " . $user['id'];
...
function addToCart($conn) {
    ...
    // ===============================
    // 1. VERIFICAR SI YA LO TIENE COMPRADO
    // ===============================
    if ($tipo === 'album') {
        $checkPurchaseSql = "SELECT ci.id_compraItem 
                             FROM CompraItem ci 
                             JOIN Compra c ON ci.id_compra = c.id_compra 
                             WHERE c.id_usuario = $userId 
                             AND ci.tipo = 'album' 
                             AND ci.id_producto = $id_producto 
                             AND c.id_estatus IN (SELECT id_estatus FROM Estatus_Compra WHERE estatus = 'Completado')";
        
        $purchaseResult = $conn->query($checkPurchaseSql);
        
        if ($purchaseResult && $purchaseResult->num_rows > 0) {
            echo json_encode([
                "success" => false,
                "message" => "Ya has comprado este álbum. Visita tu biblioteca para verlo.",
    ...
    $checkCartSql = "SELECT id_carritoItem, cantidad FROM CarritoItem 
                     WHERE id_carrito = $cartId 
                     AND tipo = '$tipo' 
                     AND id_producto = $id_producto";
    
    $checkResult = $conn->query($checkCartSql);
    
    if ($checkResult && $checkResult->num_rows > 0) {
        // Si ya está en el carrito, no permitir agregar más
        if ($tipo === 'album') {
            echo json_encode([
                "success" => false,
                "message" => "Este álbum ya está en tu carrito. No puedes agregarlo más de una vez.",
                "code" => "ALREADY_IN_CART"
            ]);
        } else {
            // Para canciones, aumentar cantidad normalmente
    ...
    // ===============================
    // 3. AGREGAR NUEVO ITEM AL CARRITO
    // ===============================
    ...
    if ($conn->query($insertSql)) {
        echo json_encode([
            "success" => true,
            "message" => $tipo === 'album' ? "Álbum agregado al carrito" : "Canción agregada al carrito",
            "action" => "added"
        ]);
    } else {
...
function getUsersStats($conn) {
    ...
    $sqlAdmin = "SELECT COUNT(*) as admins FROM usuarios WHERE role = 'admin'";
    ...
    $sqlToday = "SELECT COUNT(*) as today FROM usuarios WHERE DATE(created_at) = CURDATE()";
...
// =======================
function getSongsStats($conn) {
    $sql = "SELECT COUNT(*) as total FROM Cancion";
    $result = $conn->query($sql);
...
// =======================
function getPurchasesStats($conn) {
    // Total de compras
    $sql = "SELECT COUNT(*) as total, SUM(total) as total_sales FROM Compra 
            WHERE id_estatus IN (SELECT id_estatus FROM Estatus_Compra WHERE estatus = 'Completado')";
    $result = $conn->query($sql);
    ...
    // Compras hoy
    $sqlToday = "SELECT COUNT(*) as today FROM Compra 
                 WHERE DATE(fecha) = CURDATE() 
                 AND id_estatus IN (SELECT id_estatus FROM Estatus_Compra WHERE estatus = 'Completado')";
    $resultToday = $conn->query($sqlToday);
...
function getRecentActivity($conn) {
    ...
    // 1. Compras recientes
    $sqlPurchases = "SELECT c.id_compra, c.fecha, c.total, 
                            u.nombre as user_name, 
                            'compra' as action,
                            'Completado' as status,
                            CONCAT('Compra #', c.id_compra) as product_name
                     FROM Compra c
                     JOIN usuarios u ON c.id_usuario = u.id
                     WHERE c.id_estatus IN (SELECT id_estatus FROM Estatus_Compra WHERE estatus = 'Completado')
                     ORDER BY c.fecha DESC
                     LIMIT 5";
    
    $resultPurchases = $conn->query($sqlPurchases);
    if ($resultPurchases && $resultPurchases->num_rows > 0) {
    ...
    // 2. Registros recientes de usuarios
    $sqlUsers = "SELECT nombre, email, created_at 
                 FROM usuarios 
...
function getAllUsers($conn) {
    $sql = "SELECT id, nombre, apellido, email, telefono, fecha_nac, 
...
function updateUser($conn) {
    ...
    if (isset($data['apellido'])) $updates[] = "apellido = '" . $conn->real_escape_string($data['apellido']) . "'";
    if (isset($data['email'])) $updates[] = "email = '" . $conn->real_escape_string($data['email']) . "'";
    if (isset($data['telefono'])) $updates[] = "telefono = '" . $conn->real_escape_string($data['telefono']) . "'";
    if (isset($data['fecha_nac'])) $updates[] = "fecha_nac = '" . $conn->real_escape_string($data['fecha_nac']) . "'";
    if (isset($data['role'])) $updates[] = "role = '" . $conn->real_escape_string($data['role']) . "'";
    if (isset($data['password'])) $updates[] = "password = '" . $conn->real_escape_string($data['password']) . "'";
    ...
    $sql = "UPDATE usuarios SET " . implode(', ', $updates) . " WHERE id = $userId";
...
function getAllPurchases($conn) {
    $sql = "SELECT c.id_compra, c.fecha, c.total, c.id_estatus,
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $itemsSql = "SELECT ci.tipo, ci.id_producto, ci.cantidad, ci.precio,
                            CASE 
                                WHEN ci.tipo = 'album' THEN al.titulo
                                WHEN ci.tipo = 'cancion' THEN ca.titulo
                                ELSE 'Producto desconocido'
        ...
        $row['items'] = $items;
        $row['cliente_nombre'] = $row['cliente_nombre'] . ' ' . $row['cliente_apellido'];
        $purchases[] = $row;
    }
...
// =======================
function getPurchaseDetails($conn) {
    ...
    $sql = "SELECT c.*, 
                   ...
                   t.numero_tarjeta, t.nombre_titular, t.mes_exp, t.anio_exp,
                   ec.estatus
            FROM Compra c
            JOIN usuarios u ON c.id_usuario = u.id
            JOIN Tarjeta t ON c.id_tarjeta = t.id_tarjeta
    ...
    $purchase['cliente_nombre'] = $purchase['nombre'] . ' ' . $purchase['apellido'];
...
// =======================
function updatePurchaseStatus($conn) {
    $data = json_decode(file_get_contents("php://input"), true);
    
    if (!isset($data['purchaseId']) || !isset($data['newStatus'])) {
        echo json_encode(["success" => false, "message" => "Datos incompletos"]);
        return;
    }
    ...
    // Obtener ID del nuevo estatus
    $statusSql = "SELECT id_estatus FROM Estatus_Compra WHERE estatus = '$newStatus'";
    $statusResult = $conn->query($statusSql);
    
    if (!$statusResult || $statusResult->num_rows === 0) {
        // Crear estatus si no existe
        $conn->query("INSERT INTO Estatus_Compra (estatus) VALUES ('$newStatus')");
        $newStatusId = $conn->insert_id;
    } else {
        $statusRow = $statusResult->fetch_assoc();
        $newStatusId = $statusRow['id_estatus'];
    }
    
    // Actualizar estado de la compra
    $updateSql = "UPDATE Compra SET id_estatus = $newStatusId WHERE id_compra = $purchaseId";
    
    if ($conn->query($updateSql)) {
        // Registrar histórico si es reembolso
        if ($newStatus === 'Reembolsado' && isset($data['refundReason'])) {
            $refundReason = $conn->real_escape_string($data['refundReason']);
            $historySql = "INSERT INTO Compra_Historial (id_compra, accion, descripcion) 
                          VALUES ($purchaseId, 'reembolso', '$refundReason')";
...
function generateInvoice($conn) {
    ...
    $sql = "SELECT c.*, u.*, t.numero_tarjeta, ec.estatus
            FROM Compra c
            JOIN usuarios u ON c.id_usuario = u.id
            JOIN Tarjeta t ON c.id_tarjeta = t.id_tarjeta
            JOIN Estatus_Compra ec ON c.id_estatus = ec.id_estatus
    ...
    $html = generateInvoiceHTML($purchase);
    
    // Para una implementación real, aquí se generaría el PDF
    // Por ahora devolvemos el HTML
    header('Content-Type: text/html');
    echo $html;
}

function generateInvoiceHTML($purchase) {
    $invoiceNumber = str_pad($purchase['id_compra'], 6, '0', STR_PAD_LEFT);
    $fecha = new DateTime($purchase['fecha']);
...
<head>
    ...
    <style>
        ...
        .company-name { color: #8b0000; font-size: 28px; font-weight: bold; }
...
<body>
    <div class="invoice">
        <div class="header">
            <div class="company-name">SOUNDSPACE</div>
            <p>Av. Música 123, Ciudad Digital<br>
               Tel: (55) 1234-5678 | ventas@soundspace.com</p>
        </div>
        
        <div class="invoice-info">
            <h2>FACTURA #{$invoiceNumber}</h2>
            <p><strong>Fecha:</strong> {$fecha->format('d/m/Y H:i')}</p>
            <p><strong>Estado:</strong> {$purchase['estatus']}</p>
        </div>
        
        <div class="details">
            <div>
                <h3>Información del Cliente</h3>
                <p><strong>Nombre:</strong> {$purchase['nombre']} {$purchase['apellido']}</p>
                <p><strong>Email:</strong> {$purchase['email']}</p>
                <p><strong>Teléfono:</strong> {$purchase['telefono']}</p>
            </div>
            <div>
                <h3>Información de Pago</h3>
                <p><strong>Método:</strong> Tarjeta de crédito</p>
                <p><strong>Tarjeta:</strong> **** **** **** {$purchase['numero_tarjeta']}</p>
                <p><strong>Total:</strong> \${$purchase['total']}</p>
            </div>
        </div>
        
        <div class="total">
            <p>TOTAL: <strong>\${$purchase['total']}</strong></p>
        </div>
...
function getPurchaseStats($conn) {
    ...
    // Total ventas
    $sqlTotal = "SELECT SUM(total) as total_sales FROM Compra 
                 WHERE id_estatus IN (SELECT id_estatus FROM Estatus_Compra WHERE estatus = 'Completado')";
    $resultTotal = $conn->query($sqlTotal);
    $totalSales = $resultTotal->fetch_assoc()['total_sales'] ?? 0;
    
    // Ventas hoy
    $sqlToday = "SELECT SUM(total) as today_sales FROM Compra 
                 WHERE DATE(fecha) = '$today' 
                 AND id_estatus IN (SELECT id_estatus FROM Estatus_Compra WHERE estatus = 'Completado')";
    $resultToday = $conn->query($sqlToday);
    $todaySales = $resultToday->fetch_assoc()['today_sales'] ?? 0;
    
    // Ventas ayer
    $sqlYesterday = "SELECT SUM(total) as yesterday_sales FROM Compra 
                     WHERE DATE(fecha) = '$yesterday' 
                     AND id_estatus IN (SELECT id_estatus FROM Estatus_Compra WHERE estatus = 'Completado')";
    $resultYesterday = $conn->query($sqlYesterday);
    $yesterdaySales = $resultYesterday->fetch_assoc()['yesterday_sales'] ?? 0;
    
    // Total compras
    $sqlCount = "SELECT COUNT(*) as total_purchases FROM Compra 
                 WHERE id_estatus IN (SELECT id_estatus FROM Estatus_Compra WHERE estatus = 'Completado')";
    $resultCount = $conn->query($sqlCount);
    $totalPurchases = $resultCount->fetch_assoc()['total_purchases'] ?? 0;
    
    // Compras completadas hoy
    $sqlTodayCount = "SELECT COUNT(*) as today_completed FROM Compra 
                      WHERE DATE(fecha) = '$today' 
                      AND id_estatus IN (SELECT id_estatus FROM Estatus_Compra WHERE estatus = 'Completado')";
    $resultTodayCount = $conn->query($sqlTodayCount);
    $todayCompleted = $resultTodayCount->fetch_assoc()['today_completed'] ?? 0;
    
    // Ticket promedio
    $averageTicket = $totalPurchases > 0 ? $totalSales / $totalPurchases : 0;
    ...
    echo json_encode([
        ...
        "stats" => [
            ...
            "yesterdaySales" => floatval($yesterdaySales),
            "totalPurchases" => intval($totalPurchases),
            "todayCompleted" => intval($todayCompleted),
            "averageTicket" => floatval($averageTicket)
        ]
    ]);
}
